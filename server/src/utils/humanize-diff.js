...
function humanizeChange(change) {
  ...
  // Prefer full line context when we have it
  ...
  if (b === 'umber' && a === 'o' && (place === 'Room Number' || String(label).includes('N'))) {
    return {
  ...
  if (!b && a) {
    return { headline: `Line changed — added “${a}”.`, place: 'Line', before: b, after: a, lineChanged: true };
  }
  if (b && !a) {
    return { headline: `Line changed — removed “${b}”.`, place: 'Line', before: b, after: a, lineChanged: true };
  }
...
function humanizeSummary(changes) {
...
function enrichChanges(changes) {
  return (changes || []).map(c => ({ ...c, ...humanizeChange(c) }));
}

module.exports = { humanizeChange, humanizeSummary, enrichChanges };
